...
@section('content')
	<nav class="page-breadcrumb">
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">Admin Users List<span class="float_right">
				  @if(request('search') != '')
					<a href="{{ url('/users') }}" title="Remove Filter"><button class="btn btn-warning btn-sm"><i class="fa fa-refresh" aria-hidden="true"></i> Remove Filter</button></a>
					@endif
					@if(Auth::user()->can('viewMenu:ActionUser') || Auth::user()->can('viewMenu:All') ||  Auth::user()->type == 'admin')
				  <a href="{{ url('/users/create') }}" class="btn btn-success btn-sm" title="Add New Permission">
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table class="table">
                                <thead>
                                    <tr>
                                        <th>S.NO</th><th>Name</th><th>Email</th><th>Roles</th><th>Actions</th>
                                    </tr>
                                </thead>
                                <tbody>
                                @foreach($users as $key => $item)
									...
									<td>{{ $users->firstItem() + $key }}</td>
                                        ...
                                        <td>
										@if(null != $item->roles)
											@foreach($item->roles as $role_val)
                                        ...
                                        <td>
											...
											@if(Auth::user()->can('viewMenu:ActionUser') || Auth::user()->can('viewMenu:All') ||  Auth::user()->type == 'admin')
                                            <a href="{{ url('/users/' . $item->id . '/edit') }}" title="Edit User"><button class="btn btn-primary btn-sm"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></button></a>
                                            {!! Form::open([
                                                'method' => 'DELETE',
                                                'url' => ['/users', $item->id],
