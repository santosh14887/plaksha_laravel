...
<div class="row">
            <div class="col-lg-12 grid-margin stretch-card">
              <div class="card">
			  <div class="card-header">Dispatch Report
					  <span class="float_right">
					  @if(request('search') != '' || request('dispatch_date') != '' || request('status') != '' || request('location') != '' || request('customer') != '')
					<a href="{{ url('/reports/dispatch_report') }}" title="Remove Filter"><button class="btn btn-warning btn-sm"><i class="fa fa-refresh" aria-hidden="true"></i> Remove Filter</button></a>
					@endif
					  </span></div>
                <div class="card-body">
				  {!! Form::open(['method' => 'GET', 'url' => '/reports/dispatch_report', 'class' => 'float-right', 'role' => 'search'])  !!}
                  ...
                  <div class="table-responsive">
                    <table id="customer_revenue" class="table table-hover">
					  ...
					  @endphp
							...
							@php
							$income = $profit = $expense = $fuel_amount_paid = 0;
								foreach($value->getDispatchCompleteTicket as $vals) {
									$income += $vals->income;
									$profit += $vals->profit;
									$expense += $vals->expense;
									$fuel_amount_paid += $vals->fuel_amount_paid;
								}
							@endphp
                                <tr>
								<td>{{ ++$count_statr }}</td>
								<td>{{ (null !== $value->start_location) ? ucfirst($value->start_location).' TO '.ucfirst($value->dump_location) : '-' }}</td>
                                <td>{{ (null !== $value->getCustomer) ? ucfirst($value->getCustomer->company_name) : '-' }}</td>
								<td>{{ (null !== $value->start_time) ? ucfirst($value->start_time) : '-' }}</td>
                                <!-- <td>{{ $profit }}</td> -->
                                <td>{{ $income }}</td>
                                <td>{{ $expense }}</td>
                                <td>{{ $fuel_amount_paid }}</td>
