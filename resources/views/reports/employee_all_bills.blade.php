...
<div class="row">
            <div class="col-lg-12 grid-margin stretch-card">
              <div class="card">
			  <div class="card-header">All Bills
					  <span class="float_right">
					  @if(request('search') != '' || request('issue_date') != '' || request('user_id') != '')
					<a href="{{ url('/reports/employee_all_invoices') }}" title="Remove Filter"><button class="btn btn-warning btn-sm"><i class="fa fa-refresh" aria-hidden="true"></i> Remove Filter</button></a>
					@endif
					  </span></div>
                <div class="card-body">
				  {!! Form::open(['method' => 'GET', 'url' => '/reports/employee_all_invoices', 'class' => 'float-right', 'role' => 'search'])  !!}
				   <div class="row">
						<div class="col-sm-4">
							<div class="mb-3">
							{!! Form::label('Name', 'User', ['class' => 'form-label']) !!}
							<select id="issue_filter_status" name="user_id" class="form-control selectpicker" data-live-search="true" >
                      ...
                      <tbody>
						...
						@endphp
								...
								<td id="td_{{$value->id}}">
										...
										<select id="{{$value->id}}" class="form-control update_employee_payroll_bill_status select_height">
								...
								@endif
									...
									@if($value->invoice_pdf != '')
										...
										@else
											<td>
										<i class="menu-icon fa fa-lg fa-file employee_quickbook_bill_download_pdf cursor_pointer" data-id="{{ asset('images/pdf') }}" data-rowid ="{{ $value->id }}" data-quickbookid ="{{ $value->quickbook_invoice_id }}"></i>
										<br>
										<span id="employee_all_payroll_quickbook_{{ $value->id }}"></span>
										</td>
									@endif
